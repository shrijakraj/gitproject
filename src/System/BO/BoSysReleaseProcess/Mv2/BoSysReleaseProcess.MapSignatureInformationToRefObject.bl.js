...
if(!signatureBlobsAreEmpty) {
  promise = BoFactory.createListAsync("LoSysSignatureAttribute", {}).then(
    ...
    function (loSignatureBlob) {
      ...
      if(Utils.isDefined(ApplicationContext.get('currentTour')) && Utils.isDefined(ApplicationContext.get('currentTour').getPKey())) {
        tourReferencepkey = ApplicationContext.get('currentTour').getPKey();
      }
