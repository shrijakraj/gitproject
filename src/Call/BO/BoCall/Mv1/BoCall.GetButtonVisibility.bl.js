...
switch (token) {
  case "btnComplete":
    visible = !bCallIsReadOnly && Utils.isDefined(me.getBpaMainPKey()) && !Utils.isEmptyString(me.getBpaMainPKey());
      // If DSD, check if the visit status is InProgress
      if (ApplicationContext.get('user').hasRole('TourUser')) {
        var visitStatus = me.getClbStatus();
        visible = visible && visitStatus === "InProgress";
      }    
    break;
  ...
  case "btnTakeNewOrder":
    ...
    // If DSD, check if the visit status is not planned
    if (ApplicationContext.get('user').hasRole('TourUser')) {
      visible = visible && me.getClbStatus() !== "Planned";
    }
    break;
}
